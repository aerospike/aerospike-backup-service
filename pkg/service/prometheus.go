...
func (mc *MetricsCollector) collectBackupMetrics() {
	...
	for routineName, currentStat := range runningState {
		...
		if currentStat.Full != nil {
			backupProgress.WithLabelValues(routineName, string(jobTypeFull)).Set(float64(currentStat.Full.PercentageDone))
		}

		// Update Incremental backup metric if running
		if currentStat.Incremental != nil {
			backupProgress.WithLabelValues(routineName, string(jobTypeIncremental)).Set(float64(currentStat.Incremental.PercentageDone))
		}
	}
}
