...
func (p *BackupOrchestratorImpl) Backup(
	...
	if err != nil {
		p.completionHandler.ReportBackupOutcome(routine.Name, backupType, 0, err, logger)
		return
	}
	defer release()
	...
	p.completionHandler.ReportBackupOutcome(routine.Name, backupType, duration, err, logger)
}
