...
- **Compact backups** — A `compact` flag in [backup policy](readme/dto/dto.backuppolicy.md) skips base-64 encoding
...
- **Restore-by-timestamp overrides** — Optional `source` / `source-name` and `destination` / `destination-name` fields
  in [restore-by-timestamp requests](readme/dto/dto.restoretimestamprequest.md). When both storage and destination
...
- The `prefer-racks` configuration field has moved from the [Backup routine](readme/dto/dto.backuproutine.md) to
  the [Aerospike Cluster](readme/dto/dto.aerospikecluster.md) configuration. Update your configuration accordingly.

#### New features

- Rack‑aware backup
    - A new `rack-list` field has been added to the [Backup routine](readme/dto/dto.backuproutine.md).
...
- Human‑readable timestamps in backup paths
    - A new `timestamp-format` field is available in
      the [Backup common config](readme/dto/dto.backupcommonconfig.md).
    - Allowed values:
...
- Scan policy configuration properties
    - The [Backup policy](readme/dto/dto.backuppolicy.md) adds two optional fields:
...
- The `namespaces` field in a [backup routine](readme/dto/dto.backuproutine.md) entry is now mandatory.
...
- The `bandwidth` field in the [restore policy](readme/dto/dto.restorepolicy.md) is now specified in MiB/s instead
  of bytes per second. The minimum value for this property is 8 MiB/s.
  This aligns its unit with the equivalent property in the [backup policy](readme/dto/dto.backuppolicy.md) and
...
- **Add min-part-size to Azure and GCP**:
  ...
  is now supported for both [Azure](readme/dto/dto.azurestorage.md) and [GCP](readme/dto/dto.gcpstorage.md)
...
[`/v1/restore/status/{jobId}`](api-examples.md#restore-job-status) endpoint.

See the [monitoring guide](monitoring.md#monitoring) for details.
